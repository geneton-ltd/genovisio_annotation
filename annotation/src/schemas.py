...
from annotation.src import enums, exceptions, core
...
class CNVRegionAnnotation:
    ...
    def is_overlapping(self, target_start: int, target_end: int, overlap: enums.OverlapType) -> bool:
        ...
        if overlap == enums.OverlapType.all:
            ...
            return (target_start <= self.start < target_end < self.end) or (self.start < target_start < self.end <= target_end)
    ...
    def get_overlap_with_region(self, target_start:int, target_end:int):
...
class Annotation:
    ...
    def get_genes(self, gene_type: str | None = None, overlap: enums.OverlapType = enums.OverlapType.all) -> list[dict[str, Any]]:
    ...
    def _ts_regions(self) -> list[dict[str, Any]]:
        return [reg for reg in self.hi_region if str(reg.get('Triplosensitivity Score', '')) in core.sufficient_HI_TS_scores]

    @cached_property
    def _hi_regions(self) -> list[dict[str, Any]]:
        return [reg for reg in self.hi_region if str(reg.get('Haploinsufficiency Score', '')) in core.sufficient_HI_TS_scores]

    @cached_property
    def _ts_genes(self) -> list[dict[str, Any]]:
        return [reg for reg in self.hi_gene if str(reg.get('Triplosensitivity Score', '')) in core.sufficient_HI_TS_scores]

    @cached_property
    def _hi_genes(self) -> list[dict[str, Any]]:
        return [reg for reg in self.hi_gene if str(reg.get('Haploinsufficiency Score', '')) in core.sufficient_HI_TS_scores]
    ...
    def get_ts_genes(self, overlap_type: enums.OverlapType | None) -> list[dict[str, Any]]:
        ...
        if not overlap_type:
            ...
            return [gene for gene in self._ts_genes if self.cnv.is_overlapping(gene['start'], gene['end'], overlap_type)]
    ...
    def get_hi_genes(self, overlap_type: enums.OverlapType | None) -> list[dict[str, Any]]:
        ...
        if not overlap_type:
            ...
            return [gene for gene in self._hi_genes if self.cnv.is_overlapping(gene['start'], gene['end'], overlap_type)]
    ...
    def get_common_variability_regions(self) -> list[CommonVariabilityRegion]:
        ...
        regions = [variability for variability in self.gnomad
                          if variability['start'] <= self.cnv.start and variability['end'] >= self.cnv.end and
                          variability['population'] == 'nfe' and _normalize_type(variability['svtype']) == self.cnv.cnv_type]
    ...
    def get_benign_cnvs_gs_outer(self) -> list[dict[str, Any]]:
        ...
        cnvs = [cnv for cnv in self.benign_cnv_gs_outer if
    ...
    def get_overregions(self, gene_name: str) -> list[TranscriptRegion]:
        ...
        transcripts = [t for t in gene_info.get('transcript', []) if self.cnv.get_overlap_with_region(t['start'], t['end']) > 0]

        for transcript in transcripts:
            five_inside = False
            three_inside = False
            contained = False
            cds = [length for t in transcript.get('CDS', []) if (length := self.cnv.get_overlap_with_region(t['start'], t['end'])) ]
            five_prime_utr = [length for t in transcript.get('five_prime_UTR', []) if (length := self.cnv.get_overlap_with_region(t['start'], t['end'])) ]
            three_prime_utr = [length for t in transcript.get('three_prime_UTR', []) if (length := self.cnv.get_overlap_with_region(t['start'], t['end'])) ]
            if (is_forward and self.cnv.is_position_inside(transcript['start'])) or (not is_forward and self.cnv.is_position_inside(transcript['end'])):
                five_inside = True
            if (is_forward and self.cnv.is_position_inside(transcript['end'])) or (not is_forward and self.cnv.is_position_inside(transcript['start'])):
                three_inside = True
    ...
    def get_high_risk_loss_genes(self) -> list[dict[str, Any]]:
        ...
        for gene in self.get_genes():
            ...
            if len(risk_predictors) >= minimum_predicted:
                high_risk_genes.append(
                    {"gene_name": gene["gene_name"], "risk_predictors": risk_predictors}
                )
